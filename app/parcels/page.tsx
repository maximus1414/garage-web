    ...
    <main className="min-h-screen bg-[#0d0d0d] text-white">
      ...
      <div className="max-w-2xl mx-auto px-6 py-12">
        ...
        <p className="text-white/50 mb-8">
          Send with a driver already making the trip — cheaper than courier services
        </p>
        ...
        {submitted ? (
          <div className="bg-[#1db97a]/10 border border-[#1db97a]/30 rounded-2xl p-8 text-center">
            ...
            <p className="text-white/50 mb-6">
              We will match you with a driver going your route and send you a tracking ID.
            </p>
            <button
              onClick={() => { setSubmitted(false); setStep(1); setSelectedSize('') }}
              className="bg-[#1db97a] text-white px-6 py-3 rounded-xl font-bold hover:bg-[#149558] transition"
            >
              Send another parcel
            </button>
          </div>
          ...
          <>
            ...
            <div className="flex gap-2 mb-8">
              {[1, 2].map(s => (
                <div
                  key={s}
                  className={`h-1 flex-1 rounded-full ${s <= step ? 'bg-[#1db97a]' : 'bg-white/10'}`}
                />
              ))}
            </div>
            ...
            {step === 1 && (
              <div className="space-y-6">
                ...
                <button
                  onClick={() => setStep(2)}
                  disabled={!selectedSize || !description || !origin || !destination}
            ...
            {step === 2 && (
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <textarea
                    placeholder="Full address in destination city"
                ...
                <div className="flex gap-4">
                  <button
                    onClick={() => setStep(1)}
                    className="flex-1 border border-white/10 text-white py-4 rounded-2xl font-bold hover:bg-white/5 transition"
                  >
                    Back
                  </button>
                  <button
                    ...
                    className="flex-2 flex-1 bg-[#1db97a] text-white py-4 rounded-2xl font-bold hover:bg-[#149558] transition disabled:opacity-40"
                  >
                    Submit request
                  </button>
                </div>
