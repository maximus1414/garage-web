      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {trips.map(trip => (
              <div key={trip.id} className="bg-white/5 border border-white/10 rounded-2xl p-6 hover:border-[#1db97a]/30 transition">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h3 className="text-xl font-bold">{trip.origin} → {trip.destination}</h3>
                    <p className="text-white/40 text-sm mt-1">{trip.departure_date} · {trip.departure_time} · {trip.meeting_point}</p>
                  </div>
                  <div className="text-right">
                    <div className="text-2xl font-black text-[#1db97a]">₦{Number(trip.price_per_seat).toLocaleString()}</div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-white/40 text-xs">{trip.available_seats} seats left · {trip.vehicle_name}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    {trip.accept_cash && (
                      <span className="text-xs bg-[#fff8e6] text-[#a07000] px-3 py-1 rounded-full font-medium">Cash OK</span>
                    )}
