    ...
    <>
      <nav className="flex items-center justify-between px-6 py-4 border-b border-white/10 relative z-50">
        ...
        <div className="hidden md:block">
          <Link href="/book" className="bg-[#1db97a] text-white px-4 py-2 rounded-xl text-sm font-bold hover:bg-[#149558] transition">
            Book now
          </Link>
        </div>
      ...
      <div className={`fixed inset-0 z-40 md:hidden transition-all duration-300 ${open ? 'visible' : 'invisible'}`}>
        <div className={`absolute inset-0 bg-black/60 transition-opacity duration-300 ${open ? 'opacity-100' : 'opacity-0'}`} onClick={() => setOpen(false)} />
        <div className={`absolute right-0 top-0 h-full w-72 bg-[#0d0d0d] border-l border-white/10 transition-transform duration-300 ${open ? 'translate-x-0' : 'translate-x-full'}`}>
          ...
          <div className="flex flex-col py-4">
            {NAV_LINKS.map(link => (
              <Link key={link.href} href={link.href} onClick={() => setOpen(false)} className="px-6 py-4 text-white/70 hover:text-white hover:bg-white/5 transition border-b border-white/5 text-sm font-medium">
                {link.label}
              </Link>
            ))}
          </div>
          <div className="px-6 pt-4">
            <Link href="/book" onClick={() => setOpen(false)} className="block w-full bg-[#1db97a] text-white py-3 rounded-xl font-bold text-center hover:bg-[#149558] transition">
              Book now
            </Link>
          </div>
        </div>
